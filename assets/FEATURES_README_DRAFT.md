...
2. Run `1build/run_build.bat`.
3. Run `1build/scripts/server.bat`.
...
```bat
1build/run_build.bat
```

This runs `1build/scripts/build.py`, which currently does two main jobs:
...
The server starts from the main repo folder, so links like `players/`, `rosters/`, `assets/`, and `1build/database/` should all resolve correctly.
...
```text
1build/scripts/build.py
1build/scripts/build_players_json.py
1build/scripts/inject_css_js.py
```
...
```text
1build/database/capreport.json
```
...
3. Run `1build/run_build.bat`.
4. Run or restart `1build/scripts/server.bat`.
